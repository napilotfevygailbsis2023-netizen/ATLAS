#!/usr/bin/env python3
# ── FILE: flights.py ──────────────────────────────────────────────────────────
# CHANGES: Removed "Powered by Amadeus API" text.
#          Default date uses real today's date.
#          Book button shows toast confirmation.
#          All Luzon destinations only.
...
def render(filters: dict = None):
    ...
    <tr>
      ...
      <td><button class="btn" style="background:#0038A8;color:#fff;padding:6px 14px"
          onclick="bookFlight('{f['airline']}','{f['from']}','{f['to']}')">Book</button></td>
    </tr>""" for f in FLIGHTS)

    body = f"""
    <div class="page-wrap">
      <div style="margin-bottom:22px">
        <div class="section-title">✈️ Flight Search</div>
        ...
        <div class="card-body">
          <div class="form-row">
            <div><label class="lbl">Origin</label>
              <select class="inp">
                <option>Manila (MNL)</option><option>Cebu (CEB)</option>
              </select></div>
            ...
            <div><label class="lbl">Departure Date</label>
              <input class="inp" type="date" id="dep-date" value="{today}"/></div>
            <div><label class="lbl">Return Date</label>
              <input class="inp" type="date" id="ret-date" value="{in5}"/></div>
          ...
          <div style="text-align:right">
            <button class="btn" style="background:#0038A8;color:#fff"
              onclick="showToast('Showing available flights...')">Search Flights</button>
          </div>
        </div>
      </div>
      <div class="card">
        <div class="card-hdr" style="background:#CE1126">
          <span>Available Flights — {len(FLIGHTS)} results</span>
        </div>
    ...
    function bookFlight(airline, from, to) {{
      showToast('Booking ' + airline + ': ' + from + ' → ' + to);
    }}
    // Keep return date always after departure
