...
def render(dest="Manila", days=None, user=None):
    ...
    for day in generated:
        acts = "".join(
            ...
            f'<div style="font-size:12px;color:#94A3B8;min-width:50px;font-weight:600;padding-top:1px">{a[0]}</div>'
    ...
    <div class="page-wrap">
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-hdr" style="background:#0038A8"><span><i class="fa-regular fa-calendar"></i> Generate Itinerary</span></div>
        <div class="card-body">
      ...
      <div class="card" style="margin-bottom:24px">
        <div class="card-hdr" style="background:#1E3A5F"><span><i class="fa-solid fa-check"></i> My Saved Attractions &amp; Restaurants</span></div>
        <div class="card-body" style="padding:0">
          <div id="saved-empty" style="padding:24px;text-align:center;color:#94A3B8;font-size:14px;display:none">
            <i class="fa-solid fa-magnifying-glass"></i> No attractions added yet. Browse <a href="/attractions.py" style="color:#0038A8">Attractions</a> or <a href="/restaurants.py" style="color:#DC2626">Restaurants</a> and click "+ Add to Itinerary".
          </div>
          ...
          <div style="padding:14px 16px;border-top:1px solid #F3F4F6;background:#F9FAFB">
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
              ...
              <button class="btn" style="background:#1E3A5F;color:#fff;padding:9px 18px" onclick="manualAdd()">Add</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom:24px" id="custom-planner-card">
        <div class="card-hdr" style="background:#6B21A8;cursor:pointer" onclick="togglePlanner()">
          <span><i class="fa-solid fa-pen"></i> Custom Day Planner <span id="planner-toggle-icon" style="float:right;font-size:16px"><i class="fa-solid fa-chevron-down"></i></span></span>
        </div>
        <div id="custom-planner-body" style="padding:16px">
          <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin-bottom:16px">
            ...
            <button class="btn" style="background:#1E3A5F;color:#fff;padding:9px 18px" onclick="renderPlannerDays()">Build Plan</button>
            <button class="btn" style="background:#0038A8;color:#fff;padding:9px 18px" onclick="clearAll()"><i class="fa-regular fa-trash-can"></i> Clear All</button>
          </div>
          <div id="planner-days-container"></div>
          <div style="margin-top:16px">
            <button class="btn" style="background:#0038A8;color:#fff;padding:10px 24px" onclick="printItinerary()"><i class="fa-regular fa-square-check"></i> Print / Save PDF</button>
          </div>
        </div>
      </div>
    ...
    function renderSaved() {{
      ...
      list.innerHTML = items.map(function(item, i) {{
        ...
        var typeIcon  = item.type === 'restaurant' ? '<i class="fa-solid fa-utensils"></i>' : item.type === 'custom' ? '<i class="fa-solid fa-pen-to-square"></i>' : '<i class="fa-solid fa-building"></i>';
        return (
          ...
          '<div style="flex:1">' +
            ...
            '<div style="font-size:12px;color:#475569"><i class="fa-solid fa-location-dot"></i> ' + (item.city||'') + '</div>' +
          '</div>' +
          '<div style="display:flex;align-items:center;gap:8px">' +
            '<div>' +
              '<label style="font-size:10px;color:#94A3B8;display:block">Time</label>' +
            ...
            '</div>' +
            '<div>' +
              '<label style="font-size:10px;color:#94A3B8;display:block">Day #</label>' +
            ...
            '<div style="flex:1;min-width:120px">' +
              '<label style="font-size:10px;color:#94A3B8;display:block">Note</label>' +
            ...
            '</div>' +
            '<button onclick="removeItem(' + i + ')" style="background:#FEE2E2;color:#DC2626;border:none;border-radius:6px;padding:6px 10px;cursor:pointer;font-size:13px;font-weight:700"><i class="fa-regular fa-trash-can"></i></button>' +
          '</div></div>'
        );
      }}).join('');
    }}
    ...
    function renderPlannerDays() {{
      ...
      if (!items.length) {{
        container.innerHTML = '<div style="text-align:center;color:#94A3B8;padding:20px">No items saved yet. Add attractions above first.</div>';
        return;
      }}
      ...
      for (var d = 1; d <= ndays; d++) {{
        ...
        if (!dayItems.length) {{
          html += '<div style="font-size:13px;color:#94A3B8;padding:8px 0">No items for Day ' + d + ' yet. Set Day # on items above.</div>';
        }} else {{
          dayItems.forEach(function(item) {{
            var ic = item.type==='restaurant' ? '<i class="fa-solid fa-utensils"></i>' : item.type==='custom' ? '<i class="fa-solid fa-pen-to-square"></i>' : '<i class="fa-solid fa-building"></i>';
            html += '<div style="display:flex;gap:12px;padding:7px 0;border-bottom:1px solid #F3F4F6;align-items:flex-start">';
            html += '<div style="font-size:12px;color:#94A3B8;min-width:52px;font-weight:600;padding-top:2px">' + (item.time||'') + '</div>';
            html += '<div style="font-size:13px;color:#374151;flex:1"><span style="margin-right:4px">' + ic + '</span><strong>' + item.name + '</strong>';
            if (item.city) html += ' <span style="font-size:11px;color:#94A3B8">· ' + item.city + '</span>';
            if (item.note) html += '<div style="font-size:11px;color:#475569;margin-top:2px"><i class="fa-solid fa-note-sticky"></i> ' + item.note + '</div>';
            html += '</div></div>';
          }});
        }}
        html += '</div></div>';
    ...
    function togglePlanner() {{
      ...
      if (body.style.display === 'none') {{ body.style.display='block'; icon.innerHTML='<i class="fa-solid fa-chevron-down"></i>'; }}
      else {{ body.style.display='none'; icon.innerHTML='<i class="fa-solid fa-play"></i>'; }}
    }}
