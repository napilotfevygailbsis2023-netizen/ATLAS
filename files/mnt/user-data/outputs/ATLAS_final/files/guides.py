...
def _card(g, i):
    ...
    return (
        ...
        f'<div style="font-size:12px;color:rgba(255,255,255,.7)">{g["tours"]} tours</div>'
        '</div></div>'
...
def render(filter_city="All", filter_lang="All"):
    ...
    if filtered:
        guide_html = "".join(_card(g, i) for i, g in enumerate(filtered))
        count = f"{len(filtered)} guide(s) available"
    else:
        guide_html = '<div class="guide-empty"><div style="font-size:48px;margin-bottom:12px">&#129517;</div><div style="font-weight:700;font-size:18px;margin-bottom:8px;color:#374151">No Tour Guides Available Yet</div><div style="font-size:14px;color:#9CA3AF;max-width:340px;margin:0 auto;line-height:1.7">We are currently onboarding verified local guides. Check back soon or try a different city filter.</div></div>'
        count = "0 guides found"
    ...
    <div class="page-wrap">
      ...
      <div style="font-size:13px;color:#6B7280;margin-bottom:14px">{count}</div>
      {guide_html}
    </div>"""
