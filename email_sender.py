...
def send_otp_email(to_email: str, code: str) -> bool:
    """Send a 6-digit login OTP. Returns True on success."""
...
<body style="margin:0;padding:0;background:#F1F5F9;font-family:'Segoe UI',sans-serif">
  <div style="max-width:520px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <div style="padding:36px 40px">
      <div style="font-size:22px;font-weight:800;color:#0F172A;margin-bottom:8px">Your login code</div>
      <div style="font-size:15px;color:#475569;line-height:1.7;margin-bottom:28px">
        Use the code below to complete your sign-in.
        It expires in <strong>10 minutes</strong>.
      </div>
      <div style="text-align:center;margin-bottom:28px">
        <div style="display:inline-block;background:#F8FAFC;border:2px solid #1E3A5F;border-radius:14px;padding:20px 40px">
          <div style="font-size:11px;color:#94A3B8;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px">Login Code</div>
          <div style="font-size:42px;font-weight:900;color:#1E3A5F;letter-spacing:10px;font-family:monospace">{code}</div>
        </div>
      </div>
      <div style="font-size:13px;color:#94A3B8;line-height:1.6;border-top:1px solid #F1F5F9;padding-top:20px">
        If you didn't try to sign in to ATLAS, you can safely ignore this email.
      </div>
    </div>
    <div style="background:#F8FAFC;padding:18px 40px;text-align:center;border-top:1px solid #E2E8F0">
      <div style="font-size:12px;color:#94A3B8">&copy; 2026 ATLAS. All Rights Reserved.</div>
    ...
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Your ATLAS login code: {code}"
    msg["From"]    = f"ATLAS Travel <{GMAIL_USER}>"
    msg["To"]      = to_email
    msg.attach(MIMEText(html_body, "html"))
