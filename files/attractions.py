#!/usr/bin/env python3
# ── FILE: attractions.py ──────────────────────────────────────────────────────
# CHANGES: Removed "Powered by OpenTripMap API" text.
#          Fixed all buttons — Add to Itinerary, Nearby Restaurants, Directions
#          all show proper toast/navigation feedback.
...
def _spot_card(s):
    ...
    name = s["name"].replace("'","&#39;")
    city = s["city"]
    maps_url = f"https://www.google.com/maps/search/{s['name'].replace(' ','+')}+{city}+Philippines"
    rest_url = f"/restaurants.py?city={city}"
    return (
        '<div class="spot-card">'
        ...
        f'<button class="btn" style="background:{col};color:#fff" onclick="addItinerary(\'{name}\')">+ Add to Itinerary</button>'
        f'<a href="{rest_url}"><button class="btn" style="background:#CE1126;color:#fff">🍽️ Nearby Restaurants</button></a>'
        f'<a href="{maps_url}" target="_blank"><button class="btn" style="background:#C8930A;color:#fff">🗺️ Get Directions</button></a>'
        '</div></div></div>'
    )
...
def render(filter_city="All", filter_cat="All", keyword=""):
    ...
    spot_cards = "".join(_spot_card(s) for s in results)
    no_results = '<div class="guide-empty"><div style="font-size:40px;margin-bottom:10px">🔍</div><div style="font-weight:700;font-size:16px;margin-bottom:6px">No attractions found</div><div style="font-size:13px">Try a different city or category</div></div>' if not results else ""

    body = f"""
    <div class="page-wrap">
      <div style="margin-bottom:22px">
        <div class="section-title">🏛️ Tourist Attractions</div>
        <div class="section-sub">Discover the best tourist spots across Luzon</div>
      </div>
      <div class="card" style="margin-bottom:20px">
        <div class="card-hdr" style="background:#CE1126"><span>Filter & Search</span></div>
        <div class="card-body">
          <form method="get" style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-end">
            ...
            <div style="flex:1;min-width:180px"><label class="lbl">Keyword Search</label>
      ...
      {spot_cards}{no_results}
    </div>
    <script>
    function addItinerary(name) {{
      showToast('✓ Added to your itinerary: ' + name);
    }}
    </script>"""
